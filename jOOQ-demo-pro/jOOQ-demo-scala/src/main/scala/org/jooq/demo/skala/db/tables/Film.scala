...
extends TableImpl[FilmRecord](
   ...
   */
  val LAST_UPDATE: TableField[FilmRecord, LocalDateTime] = createField(DSL.name("last_update"), SQLDataType.LOCALDATETIME(6).nullable(false).readonly(true).defaultValue(DSL.field(DSL.raw("now()"), SQLDataType.LOCALDATETIME)), "")
  ...
  override def getTriggers: List[Trigger] = Arrays.asList[Trigger](Triggers.FILM_FULLTEXT_TRIGGER)
