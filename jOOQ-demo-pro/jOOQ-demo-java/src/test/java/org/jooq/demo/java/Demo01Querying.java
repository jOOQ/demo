    ...
    public void existsConvenience() {
        ...
        println(ctx
            .select(
                ...
                CUSTOMER.paymentExists(p -> p.where(p.AMOUNT.gt(new BigDecimal("0.9")))).as("has payments > 0.9"))
            .from(CUSTOMER)
            .fetch());
        ...
        println(ctx
            .select(
                ...
                CUSTOMER.LAST_NAME)
            .from(CUSTOMER)
            .where(CUSTOMER.paymentExists(p -> p.where(p.AMOUNT.gt(new BigDecimal("0.9")))))
            .fetch());
        /* [/pro] */
    }
}
