...
import org.jooq.demo.db.tables.records.SalesByFilmCategoryRecord;
...
public class SalesByFilmCategory extends TableImpl<SalesByFilmCategoryRecord> {
    ...
    private SalesByFilmCategory(Name alias, Table<SalesByFilmCategoryRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"sales_by_film_category\" as SELECT c.name AS category, sum(p.amount) AS total_sales FROM (((((sakila.public.payment AS p JOIN sakila.public.rental AS r ON ((p.rental_id = r.rental_id))) JOIN sakila.public.inventory AS i ON ((r.inventory_id = i.inventory_id))) JOIN sakila.public.film AS f ON ((i.film_id = f.film_id))) JOIN sakila.public.film_category AS fc ON ((f.film_id = fc.film_id))) JOIN sakila.public.category AS c ON ((fc.category_id = c.category_id))) GROUP BY c.name ORDER BY sum(p.amount) DESC"));
    }
