...
import org.jooq.demo.db.tables.records.SalesByStoreRecord;
...
public class SalesByStore extends TableImpl<SalesByStoreRecord> {
     ...
     */
    public final TableField<SalesByStoreRecord, String> STORE = createField(DSL.name("store"), SQLDataType.VARCHAR, this, "");

    /**
     * The column <code>public.sales_by_store.manager</code>.
     */
    public final TableField<SalesByStoreRecord, String> MANAGER = createField(DSL.name("manager"), SQLDataType.VARCHAR, this, "");
    ...
    private SalesByStore(Name alias, Table<SalesByStoreRecord> aliased, Field<?>[] parameters) {
        super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("create view \"sales_by_store\" as SELECT (((c.city)::STRING || ','::STRING) || (cy.country)::STRING) AS store, (((m.first_name)::STRING || ' '::STRING) || (m.last_name)::STRING) AS manager, sum(p.amount) AS total_sales FROM (((((((sakila.public.payment AS p JOIN sakila.public.rental AS r ON ((p.rental_id = r.rental_id))) JOIN sakila.public.inventory AS i ON ((r.inventory_id = i.inventory_id))) JOIN sakila.public.store AS s ON ((i.store_id = s.store_id))) JOIN sakila.public.address AS a ON ((s.address_id = a.address_id))) JOIN sakila.public.city AS c ON ((a.city_id = c.city_id))) JOIN sakila.public.country AS cy ON ((c.country_id = cy.country_id))) JOIN sakila.public.staff AS m ON ((s.manager_staff_id = m.staff_id))) GROUP BY cy.country, c.city, s.store_id, m.first_name, m.last_name ORDER BY cy.country, c.city"));
    }
